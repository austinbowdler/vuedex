<template>
  <div v-if="pokemonData && pokemonDescription" class="pokemon-card nes-container is-rounded">
    <div class="pokemon-card__image">
      <img class="sprite" :src="`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemonData.id}.png`" :alt="pokemonData.name">
    </div>
    <div class="pokemon-card__text">
      ...
      <p>Abilities:
        <span v-for="(abilitiesOfPokemon, index) in pokemonData.abilities" :key="`type${index}`">
          {{ abilitiesOfPokemon.ability.name }} / 
        </span>
      </p>
      <p>Base Stats:
        <span v-for="(statsOfPokemon, index) in pokemonData.stats" :key="`type${index}`">
          {{ statsOfPokemon.stat.name }} - {{ statsOfPokemon.base_stat }}/ 
        </span>
      </p>
